...
### 15.6 Grup Field Dinamis / Repeater (engine v1.2.0, schema v1.1.0)

**Problem it solves:** some real-world forms need a field group that can be duplicated an unknown number of times — the motivating case is *1 rumah bisa punya banyak Kepala Keluarga* (a house can have several household heads, each with their own No. KK + Nama KK, unbounded). Before schema v1.1.0, FormGear's `panel` type was a purely visual, single-instance container with no way to duplicate itself or its children.
...
**Builder ergonomics (built on top of the flag):** rather than only being able to hand-configure a *new* panel as repeatable, a section toolbar button *"Pilih Field untuk Grup Berulang"* puts that section into a selection mode (`this.groupSelection`): a checkbox appears next to **every** field in the section — top-level *and* nested child fields, at any depth — plus an inline bar to type a group name and an item label. *"Jadikan Grup Berulang"* (`createRepeaterGroupFromSelection()`) then creates a new `repeatable` panel and moves the checked fields into it as `children`. The reverse operation, *"Ubah jadi Field Biasa (Ungroup)"* (`ungroupRepeaterField()`), splices a repeatable panel's children back out to its parent's level and deletes the panel.

**Mixed-depth selection & precise conflict resolution (`createRepeaterGroupFromSelection()`):** because a checked field can now be a nested child whose visibility depends on `visibleIfValue` relative to its *immediate* choice-type parent, moving it can break that relationship if the parent doesn't move with it. This is resolved deterministically in five steps, entirely by field identity rather than by restricting what can be selected:

1. **Normalize to "roots".** Any checked path that has an *ancestor* also checked is dropped from further processing — it isn't an independent move, it simply travels along as part of that ancestor's subtree (still nested under it, in the exact same parent/child object, once the ancestor moves). Checking a parent and one of its own children at the same time is therefore harmless: the child checkbox becomes redundant and the UI marks it *"Ikut otomatis (induk sudah dipilih)"* (disabled, pre-checked) once its ancestor is checked.
2. **Resolve object references before any mutation.** For each remaining root path, the actual field object and its current parent container (`section.fields` or some ancestor's `children` array) are looked up once via `getFieldByPath()`. From this point on, everything operates on those object/array references — never on the path strings again — so later removals elsewhere in the tree can never invalidate an already-resolved reference or shift it out from under an index-based lookup.
3. **Reserve the insertion point with a placeholder.** A sentinel object is spliced into `section.fields` at the position of the earliest-selected top-level branch *before* any of the actual removals happen. Because it's a real array element, it rides along naturally through every subsequent `splice()` anywhere else in the tree, so its position always reflects "where the group should end up" regardless of how many fields get removed, at how many different depths, in what order.
4. **Extract each root by object identity** (`parentContainer.indexOf(field)`, not a remembered numeric index — indices from other removals may have shifted). **Only the root field's own `visibleIfValue` is deleted** at this point — never a descendant's — because only the root itself changes immediate parent (from whatever it was, to the new panel, which is never a choice type). Any deeper field that came along as part of a root's subtree (case 1) keeps its own `visibleIfValue` completely untouched, since its immediate parent inside that subtree hasn't changed at all.
5. **Replace the placeholder** with the finished repeatable panel (`section.fields.indexOf(placeholder)`, falling back to a plain push if it was somehow already gone).

The net effect: selecting a choice-type field together with its own `visibleIfValue`-conditioned children keeps that whole conditional subtree intact and working, nested exactly as before, inside the new group. Selecting a child *without* its parent correctly and automatically strips that one field's now-meaningless `visibleIfValue`, and nothing else is touched. `ungroupRepeaterField()` applies the same "only strip the field(s) actually changing immediate parent" principle when splicing a group's children back out.

- Nesting a repeatable panel's children further (e.g. a choice field inside a group whose *own* children have `visibleIfValue`) works transparently at the data/render level, since the recursive renderer and `updateConditionalVisibility()` walk are repeater-aware (see below) — grouping a choice field together with its conditional children (step 1 above) produces exactly this structure.
